...
2. Probability laws: describe beliefs about likelyhood of outcomes (or assign probability to outcomes)

### Sample space:
list of possible outcomes, must be mutually exclusive, collective exhaustive, and at the right granularity.
...
> Motivation:
> Assign individual probability to single dot is challenge (in the dart example, each dot have zero probability), so we assign probability to the subset of the sample space.
...
### Concequences of axioms (properties of probability axioms)
...
### Counable Additivity Axiom (refined probability axiom)
...
To refine the 3rd probability axiom, additivity hold only for "countable" sequences of events. That means the additivity axiom must be a sequence, with finite or infinite elements.

Note the following contradiction, so the additivity axiom can not be applied to continuous sample spaece.

$A_1, A_2, A_3, \cdots$ are real coordinates, if you apply the additivity axiom, you will get:
$1={\bf P}(\Omega )={\bf P}\big (\{ A_1\} \cup \{ A_2\} \cup \{ A_3\} \cdots \big ) ={\bf P}(\{ A_1\} )+{\bf P}(\{ A_2\} )+{\bf P}(\{ A_3\} )+\cdots = 0+0+0+\cdots =0$, which is contradict.
...
* Probability are offen intepreted as:
...
Also applies to infinite countable sets according to the countable additivity axiom.

### Baye's rule

The setting is same as the total probability theorem.
...
* A random variable ("r.v.") associate a value (a number) to every possible outcome.
* Mathematically, A function from sample space $\Omega$ to the real numbers (discrete or continuous).
* We can have serveral random variables defined on the same sample space. (In Lecture 18, $X_1, X_2, \cdots$ are independent random variables from the same distribution)
* A function of one or several random variables is also another random variable.
* A concrete example to understand the concept of random variable is take a class of students, selecting students and measuring their body mass or height will give us the random variable $W$ and $H$. The "Body mass index" is another random variable that is a function of the random variable $W$ and $H$.
...
* Second moment of exponential random variable
...
### Conditioning a continuous random varaible on an event
...
* The moral of this problem is that an estimate of $ \Theta_1 $ obtained by indentifying the maximum of the join PMF of all unknown random variable  can be different from the MAP estimate of $\Theta_1$ from the marginal PMF.
